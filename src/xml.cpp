...
#ifndef LINUX
XML :: XML(const wchar_t* file,int LoadMode,class XMLTransform* eclass,class XMLTransformData* edata)
   {
   Init();
   Load((char*)file,7,eclass,edata);
   }
#endif
...
int XMLElement :: GetElementName(char* x,int NoDecode)
	{
	if (!x)
		{
		if (NoDecode)
...
int XMLElement :: GetElementFullName(char* x,int NoDecode)
	{
...
XMLElement* XMLElement:: FindElementZ(char* n,bool ForceCreate,char* el,bool Temp)
   {
   for(unsigned int i = 0 ; i < childrennum ; i++)
      {
		if (!children[i])
...
XMLVariable* XMLElement :: FindVariableZ(char*  x,bool ForceCreate,char* defnew,bool Temp)
   {
   for(unsigned int i = 0 ; i < variablesnum ; i++)
      {
      int Sug = variables[i]->GetName(0);
...
int XML :: Load(const char* file,int LoadMode,XMLTransform* eclass,class XMLTransformData* edata)
   ...
   if (LoadMode == 7) // local xml file unicode
      {
...
// XMLContent class
XMLContent :: XMLContent(XMLElement* p,int ElementPosition,const char* ht,int NoDecode)
   {
   parent = p;
   ep = ElementPosition;
   c = 0;
...
int XMLContent :: GetValue(char* x,int NoDecode)
	{
	if (!x)
	  {
	  if (NoDecode)
...
int XMLVariable :: GetName(char* x,int NoDecode)
   {
   if (!x)
      {
      if (NoDecode)
...
int XMLVariable :: GetValue(char* x,int NoDecode)
   {
   if (!x)
      {
      if (NoDecode)
